...
for inspection and for pre-flashing a ZuluProfiles partition image. With
--header it also emits the blob as a C array: the firmware build uses that
(src/generate-embedded-profiles.py) to compile a fallback store into the
firmware image itself.

See ZPDB_FORMAT.md for the on-flash layout.

Usage:
    python zpdb_build.py [-o out.bin] [--header out.h [--section name]] [--report]
                         file.ini [...]
"""

# Emitted at the top of the --header output. Kept below the module docstring so
# that __doc__ -- which the argparse description is built from -- stays set.
copyright = \
'''
/**
 * ZuluSCSI™ - Copyright (c) 2026 Rabbit Hole Computing™
...
def main():
    ...
    ap.add_argument("--section", help="linker section to place the --header array in")
    ...
    if args.header:
        with open(args.header, "w", encoding='utf-8') as fh:
            ...
            attr = ""
            if args.section:
                attr = ' __attribute__((section("%s")))' % args.section
            fh.write("alignas(4) const uint8_t %s[]%s = {\n" % (args.symbol, attr))
